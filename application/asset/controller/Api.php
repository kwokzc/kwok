...
use app\common\model\AssetAccountType as AccountType;
...
//                exit(json_encode($tmp));
     ...
     * 增加资产账户分类
     */
    public function addAssetAccountType(){
        //判断是否是Ajax发送
        if (Request::isAjax()){
            ...
            if (true !== $res){
                ...
                //成功则向数据库写入
                if ($res = AccountType::create($data)){
                    //写入成功再获取一次
                    $data = AccountType::get($res->id);
